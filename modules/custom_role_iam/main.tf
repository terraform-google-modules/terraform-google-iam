...
locals {
  valid_target_id      = coalesce(var.project_id, var.target_id)
...
/******************************************
  Validating one of project_id or target_id is available
 *****************************************/

resource "null_resource" "validate_project_id" {
  count = local.valid_target_id == null ? 1 : 0

  provisioner "local-exec" {
    command = "echo 'Either project_id or target_id must be provided' && exit 1"
  }
}
...
data "google_iam_testable_permissions" "unsupported_permissions" {
  full_resource_name   = var.target_level == "org" ? "//cloudresourcemanager.googleapis.com/organizations/${local.valid_target_id}" : "//cloudresourcemanager.googleapis.com/projects/${local.valid_target_id}"
...
resource "google_organization_iam_member" "custom_role_member" {
  ...
  org_id   = local.valid_target_id
  role     = "organizations/${local.valid_target_id}/roles/${local.custom-role-output}"
  member   = each.key
}
...
resource "google_project_iam_member" "custom_role_member" {
  ...
  project  = local.valid_target_id
  role     = "projects/${local.valid_target_id}/roles/${local.custom-role-output}"
  member   = each.key
}
